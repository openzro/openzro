...
// ADR-0022 D4b. Wire-compatible with older agents: the default
// (UNSPECIFIED = 0) means "no information, treat as legacy".
type CustomZoneSource int32

const (
	CustomZoneSource_CUSTOM_ZONE_SOURCE_UNSPECIFIED CustomZoneSource = 0
	CustomZoneSource_CUSTOM_ZONE_SOURCE_PEERS       CustomZoneSource = 1
	CustomZoneSource_CUSTOM_ZONE_SOURCE_USER        CustomZoneSource = 2
)

// Enum value maps for CustomZoneSource.
var (
	CustomZoneSource_name = map[int32]string{
		0: "CUSTOM_ZONE_SOURCE_UNSPECIFIED",
		1: "CUSTOM_ZONE_SOURCE_PEERS",
		2: "CUSTOM_ZONE_SOURCE_USER",
	}
	CustomZoneSource_value = map[string]int32{
		"CUSTOM_ZONE_SOURCE_UNSPECIFIED": 0,
		"CUSTOM_ZONE_SOURCE_PEERS":       1,
		"CUSTOM_ZONE_SOURCE_USER":        2,
	}
)

func (x CustomZoneSource) Enum() *CustomZoneSource {
	p := new(CustomZoneSource)
	*p = x
	return p
}

func (x CustomZoneSource) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (CustomZoneSource) Descriptor() protoreflect.EnumDescriptor {
	return file_management_proto_enumTypes[0].Descriptor()
}

func (CustomZoneSource) Type() protoreflect.EnumType {
	return &file_management_proto_enumTypes[0]
}

func (x CustomZoneSource) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use CustomZoneSource.Descriptor instead.
func (CustomZoneSource) EnumDescriptor() ([]byte, []int) {
...
type OpenzroConfig struct {
	...
	// operator-set desktop client self-update directive (openZro #5,
	// management-driven model). Conveyed to every peer over the existing
	// Sync stream; the peer obeys, then runs its own
	// download+verify+install pipeline (management never supplies the
	// binary). Clean-room: modelled from public NetBird behaviour
	// (v0.61 client auto-update), no upstream AGPL management consulted.
...
func (x *OpenzroConfig) GetUpdate() *UpdateConfig {
	if x != nil {
		return x.Update
	}
	return nil
...
// UpdateConfig is the per-account client-update directive an operator
// sets in the dashboard. Empty target_version => no directive. force
// => install silently in the background; otherwise the client
// surfaces a prompt.
...
func (x *UpdateConfig) GetTargetVersion() string {
	if x != nil {
...
func (x *UpdateConfig) GetForce() bool {
	if x != nil {
		return x.Force
	}
	return false
...
type FlowConfig struct {
	...
	// intersect this list. Empty list means "all peers report" (the
	// default). Set by management from the account's
	// ExtraSettings.FlowEventsGroups; the peer enforces the filter
	// locally so excluded peers never spend CPU on capture or push
	// events to management.
	Groups []string `protobuf:"bytes,9,rep,name=groups,proto3" json:"groups,omitempty"`
	// disableDefaultPortFilter turns off the client's built-in skip
	// list of broadcast / discovery ports (SSDP-1900, mDNS-5353,
	// NetBIOS-137/138, LLMNR-5355). Off by default: a corporate VPN
	// typically does NOT want these events polluting telemetry,
	// they're high-volume "device A is here" chatter that no audit
	// policy cares about. Operators who explicitly want to track
	// discovery traffic flip this to true and the built-in skip
	// list is ignored.
	DisableDefaultPortFilter bool `protobuf:"varint,10,opt,name=disableDefaultPortFilter,proto3" json:"disableDefaultPortFilter,omitempty"`
	// excludedPorts is an operator-defined list of (port, protocol)
	// pairs to drop on the client side, ADDED to the built-in skip
	// list (unless disableDefaultPortFilter is true, in which case
	// ONLY this list applies). Useful for environments with
	// additional protocols generating uninteresting flow events
	// (e.g. internal heartbeats, custom multicast).
	ExcludedPorts []*FlowPortFilter `protobuf:"bytes,11,rep,name=excludedPorts,proto3" json:"excludedPorts,omitempty"`
}
...
func (x *FlowConfig) GetDisableDefaultPortFilter() bool {
	if x != nil {
		return x.DisableDefaultPortFilter
	}
	return false
}

func (x *FlowConfig) GetExcludedPorts() []*FlowPortFilter {
	if x != nil {
...
// FlowPortFilter describes a single (port, protocol) pair the client
// drops at the conntrack-event boundary before queueing for the
// management. protocol is "tcp" / "udp" / "any" (matches both).
...
func (x *FlowPortFilter) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*FlowPortFilter) ProtoMessage() {}

func (x *FlowPortFilter) ProtoReflect() protoreflect.Message {
	mi := &file_management_proto_msgTypes[18]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use FlowPortFilter.ProtoReflect.Descriptor instead.
func (*FlowPortFilter) Descriptor() ([]byte, []int) {
	return file_management_proto_rawDescGZIP(), []int{18}
}

func (x *FlowPortFilter) GetPort() uint32 {
	if x != nil {
		return x.Port
	}
	return 0
...
func (x *FlowPortFilter) GetProtocol() string {
	if x != nil {
		return x.Protocol
	}
	return ""
...
// CustomZone represents a dns.CustomZone — operator-managed
// authoritative DNS namespace distributed to peers in selected
// groups (issue #108, ADR-0022 Phase 2). Two new fields are
// additive: older agents that don't know about them read zero
// values and behave as today (zone always added to search list,
...
type CustomZone struct {
	...
	// SearchDomainEnabled — when true, the agent appends the zone's
	// Domain to the OS DNS search list (so a bare-name lookup like
	// `db` resolves as `db.<Domain>`). The synthetic peer zone is
	// emitted with this flag = true to preserve current peer-name
	// resolution behavior; user-managed zones default to false
	// (operator opts in per zone).
	SearchDomainEnabled bool `protobuf:"varint,3,opt,name=SearchDomainEnabled,proto3" json:"SearchDomainEnabled,omitempty"`
	// Source distinguishes the synthetic peer zone (PEERS) from
	// operator-managed zones (USER). UNSPECIFIED is the default for
	// backwards compatibility with old management daemons that don't
	// populate this field; new agents treat UNSPECIFIED zones as
	// PEERS (legacy behavior).
	Source CustomZoneSource `protobuf:"varint,4,opt,name=Source,proto3,enum=management.CustomZoneSource" json:"Source,omitempty"`
}
...
func (x *CustomZone) GetSearchDomainEnabled() bool {
	if x != nil {
		return x.SearchDomainEnabled
	}
	return false
}

func (x *CustomZone) GetSource() CustomZoneSource {
	if x != nil {
		return x.Source
	}
	return CustomZoneSource_CUSTOM_ZONE_SOURCE_UNSPECIFIED
}
...
var file_management_proto_rawDesc = []byte{
	...
	0x0d, 0x6f, 0x70, 0x65, 0x6e, 0x7a, 0x72, 0x6f, 0x43, 0x6f, 0x6e, 0x66, 0x69, 0x67, 0
	0x6e, 0x53, 0x69, 0x67, 0x6e, 0x61, 0x74, 0x75, 0x72, 0x65, 0x22, 0xc3, 0x03, 0x0a, 0x0a, 0x46,
	...
	,
	...
	0x6f, 0x6c, 0x22, 0x7d, 0x0a, 0x13, 0x50, 0x72, 0x6f, 0x74, 0x65, 0x63, 0x74, 0x65, 0x64, 0x48,
	...
	0x64, 0x22, 0x81, 0x02, 0x0a, 0x0a, 0x50, 0x65, 0x65, 0x72, 0x43, 0x6f, 0x6e, 0x66, 0x69, 0x67,
	...
	0x69, 0x6e, 0x46, 0x6c, 0x61, 0x67, 0x22, 0xed, 0x01, 0x0a, 0x05, 0x52, 0x6f, 0x75, 0x74, 0x65,
	...
	0x52, 0x0b, 0x43, 0x75, 0x73, 0x74, 0x6f, 0x6d, 0x5a, 0x6f, 0x6e, 0x65, 0x73, 0x22, 0xc0, 0x01,
	...
	0x07, 0x52, 0x65, 0x63, 0x6f, 0x72, 0x64, 0x73, 0x12, 0x30, 0x0a, 0x13, 0x53, 0x65, 0x61, 0x72,
	0x63, 0x68, 0x44, 0x6f, 0x6d, 0x61, 0x69, 0x6e, 0x45, 0x6e, 0x61, 0x62, 0x6c, 0x65, 0x64, 0x18,
	0x03, 0x20, 0x01, 0x28, 0x08, 0x52, 0x13, 0x53, 0x65, 0x61, 0x72, 0x63, 0x68, 0x44, 0x6f, 0x6d,
	0x61, 0x69, 0x6e, 0x45, 0x6e, 0x61, 0x62, 0x6c, 0x65, 0x64, 0x12, 0x34, 0x0a, 0x06, 0x53, 0x6f,
	0x75, 0x72, 0x63, 0x65, 0x18, 0x04, 0x20, 0x01, 0x28, 0x0e, 0x32, 0x1c, 0x2e, 0x6d, 0x61, 0x6e,
	0x61, 0x67, 0x65, 0x6d, 0x65, 0x6e, 0x74, 0x2e, 0x43, 0x75, 0x73, 0x74, 0x6f, 0x6d, 0x5a, 0x6f,
	0x6e, 0x65, 0x53, 0x6f, 0x75, 0x72, 0x63, 0x65, 0x52, 0x06, 0x53, 0x6f, 0x75, 0x72, 0x63, 0x65,
	...
	,
...
var file_management_proto_goTypes = []interface{}{
	(CustomZoneSource)(0),                  // 0: management.CustomZoneSource
	...
	(*OpenzroConfig)(nil),                  // 19: management.OpenzroConfig
	(*UpdateConfig)(nil),                   // 20: management.UpdateConfig
	(*HostConfig)(nil),                     // 21: management.HostConfig
	(*RelayConfig)(nil),                    // 22: management.RelayConfig
	(*FlowConfig)(nil),                     // 23: management.FlowConfig
	(*FlowPortFilter)(nil),                 // 24: management.FlowPortFilter
	...
	(*PortInfo_Range)(nil),                 // 47: management.PortInfo.Range
...
var file_management_proto_depIdxs = []int32{
	...
	19, // 1: management.SyncResponse.openzroConfig:type_name -> management.OpenzroConfig
	...
	19, // 13: management.LoginResponse.openzroConfig:type_name -> management.OpenzroConfig
	26, // 14: management.LoginResponse.peerConfig:type_name -> management.PeerConfig
	43, // 15: management.LoginResponse.Checks:type_name -> management.Checks
	48, // 16: management.ServerKeyResponse.expiresAt:type_name -> google.protobuf.Timestamp
	21, // 17: management.OpenzroConfig.stuns:type_name -> management.HostConfig
	25, // 18: management.OpenzroConfig.turns:type_name -> management.ProtectedHostConfig
	21, // 19: management.OpenzroConfig.signal:type_name -> management.HostConfig
	22, // 20: management.OpenzroConfig.relay:type_name -> management.RelayConfig
	23, // 21: management.OpenzroConfig.flow:type_name -> management.FlowConfig
	20, // 22: management.OpenzroConfig.update:type_name -> management.UpdateConfig
	4,  // 23: management.HostConfig.protocol:type_name -> management.HostConfig.Protocol
	49, // 24: management.FlowConfig.interval:type_name -> google.protobuf.Duration
	24, // 25: management.FlowConfig.excludedPorts:type_name -> management.FlowPortFilter
	21, // 26: management.ProtectedHostConfig.hostConfig:type_name -> management.HostConfig
	29, // 27: management.PeerConfig.sshConfig:type_name -> management.SSHConfig
	26, // 28: management.NetworkMap.peerConfig:type_name -> management.PeerConfig
	28, // 29: management.NetworkMap.remotePeers:type_name -> management.RemotePeerConfig
	35, // 30: management.NetworkMap.Routes:type_name -> management.Route
	36, // 31: management.NetworkMap.DNSConfig:type_name -> management.DNSConfig
	28, // 32: management.NetworkMap.offlinePeers:type_name -> management.RemotePeerConfig
	41, // 33: management.NetworkMap.FirewallRules:type_name -> management.FirewallRule
	45, // 34: management.NetworkMap.routesFirewallRules:type_name -> management.RouteFirewallRule
	46, // 35: management.NetworkMap.forwardingRules:type_name -> management.ForwardingRule
	29, // 36: management.RemotePeerConfig.sshConfig:type_name -> management.SSHConfig
	5,  // 37: management.DeviceAuthorizationFlow.Provider:type_name -> management.DeviceAuthorizationFlow.provider
	34, // 38: management.DeviceAuthorizationFlow.ProviderConfig:type_name -> management.ProviderConfig
	34, // 39: management.PKCEAuthorizationFlow.ProviderConfig:type_name -> management.ProviderConfig
	39, // 40: management.DNSConfig.NameServerGroups:type_name -> management.NameServerGroup
	37, // 41: management.DNSConfig.CustomZones:type_name -> management.CustomZone
	38, // 42: management.CustomZone.Records:type_name -> management.SimpleRecord
	0,  // 43: management.CustomZone.Source:type_name -> management.CustomZoneSource
	...
	47, // 49: management.PortInfo.range:type_name -> management.PortInfo.Range
	...
	18, // 58: management.ManagementService.GetServerKey:input_type -> management.Empty
	18, // 59: management.ManagementService.isHealthy:input_type -> management.Empty
	...
	6,  // 63: management.ManagementService.Login:output_type -> management.EncryptedMessage
	6,  // 64: management.ManagementService.Sync:output_type -> management.EncryptedMessage
	17, // 65: management.ManagementService.GetServerKey:output_type -> management.ServerKeyResponse
	18, // 66: management.ManagementService.isHealthy:output_type -> management.Empty
	6,  // 67: management.ManagementService.GetDeviceAuthorizationFlow:output_type -> management.EncryptedMessage
	6,  // 68: management.ManagementService.GetPKCEAuthorizationFlow:output_type -> management.EncryptedMessage
	18, // 69: management.ManagementService.SyncMeta:output_type -> management.Empty
