...
func TestEngine_UpdateNetworkMapWithDNSUpdate(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			expectedZones: []nbdns.CustomZone{
				{
					// Synthetic peer zone arrives from the proto with
					// Source=UNSPECIFIED on a legacy fixture; toDNSConfig
					// normalizes that to SearchDomainEnabled=true so the
					// agent keeps it in the OS search list (issue #108 /
					// ADR-0022 D6 backward-compat).
					Domain: "openzro.cloud.",
				...
				{
					// In this test the reverse-DNS zone is fed through
					// the proto (not auto-generated by addReverseZone),
					// so it goes through toDNSConfig's UNSPECIFIED→true
					// normalization just like the synthetic peer zone.
					// The host configurator forces MatchOnly=true for
					// reverse zones regardless of SearchDomainEnabled,
					// so the search-domain flag is moot on the OS side
					// — but the struct equality assertion sees it.
					Domain:              "0.66.100.in-addr.arpa.",
					SearchDomainEnabled: true,
				},
			},
