...
const (
	fileGeneratedResolvConfContentHeader         = "# Generated by Openzro"
...
# The original file can be restored from ` + fileDefaultResolvConfBackupLocation + "\n\n"
	...
	fileDefaultResolvConfBackupLocation = defaultResolvConfPath + ".original.openzro"
...
func (f *fileConfigurator) updateConfig(nbSearchDomains []string, nbNameserverIP netip.Addr, cfg *resolvConf, stateManager *statemanager.Manager) error {
	...
	log.Infof("created a Openzro managed %s file with the DNS settings. Added %d search domains. Search list: %s", defaultResolvConfPath, len(searchDomainList), searchDomainList)
...
func (f *fileConfigurator) restore() error {
	...
	if err != nil {
		log.Errorf("Failed to remove openzro nameserver from %s on backup restore: %s", fileDefaultResolvConfBackupLocation, err)
	}
...
func (f *fileConfigurator) restoreUncleanShutdownDNS(storedDNSAddress *netip.Addr) error {
	...
	// current address is still openzro's non-available dns address -> restore
	// comparing parsed addresses only, to remove ambiguity
	if currentDNSAddress.String() == storedDNSAddress.String() {
		return restoreResolvConfFile()
	}

	log.Infof("restoring unclean shutdown: first current nameserver differs from saved nameserver pre-openzro: %s (current) vs %s (stored): not restoring", currentDNSAddress, storedDNSAddress)
	return nil
}
