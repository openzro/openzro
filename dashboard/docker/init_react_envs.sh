...
if [[ -z "${OPENZRO_MGMT_API_ENDPOINT}" ]]; then
    echo "OPENZRO_MGMT_API_ENDPOINT environment variable must be set"
    exit 1
fi
...
export OPENZRO_HOTJAR_TRACK_ID=${OPENZRO_HOTJAR_TRACK_ID}
export OPENZRO_GOOGLE_ANALYTICS_ID=${OPENZRO_GOOGLE_ANALYTICS_ID}
export OPENZRO_GOOGLE_TAG_MANAGER_ID=${OPENZRO_GOOGLE_TAG_MANAGER_ID}
...
echo "Openzro latest version: ${OPENZRO_LATEST_VERSION}"

# replace ENVs in the config
ENV_STR="\$\$USE_AUTH0 \$\$AUTH_AUDIENCE \$\$AUTH_AUTHORITY \$\$AUTH_CLIENT_ID \$\$AUTH_CLIENT_SECRET \$\$AUTH_SUPPORTED_SCOPES \$\$OPENZRO_MGMT_API_ENDPOINT \$\$OPENZRO_MGMT_GRPC_API_ENDPOINT \$\$OPENZRO_HOTJAR_TRACK_ID \$\$OPENZRO_GOOGLE_ANALYTICS_ID \$\$OPENZRO_GOOGLE_TAG_MANAGER_ID \$\$AUTH_REDIRECT_URI \$\$AUTH_SILENT_REDIRECT_URI \$\$OPENZRO_TOKEN_SOURCE \$\$OPENZRO_DRAG_QUERY_PARAMS"
