...
import { getLatestOpenzroRelease } from "@utils/version";
import React, {
  useCallback,
  useContext,
  useEffect,
...
import type { OpenzroRelease } from "@/interfaces/Version";
...
export default function ApplicationProvider({ children }: Props) {
  const [latestRelease, setLatestRelease] = useLocalStorage<
    OpenzroRelease | undefined
  >("openzro-latest-release", undefined);
  ...
  useEffect(() => {
    async function fetchLatestRelease() {
      const release = await getLatestOpenzroRelease(latestRelease);
