...
import { OpenzroRelease } from "@/interfaces/Version";

const GITHUB_API_ENDPOINT = "https://api.github.com";
const LATEST_RELEASE_CHECK_INTERVAL_IN_MINUTES = 10;

export const getLatestOpenzroRelease = async (
  release?: OpenzroRelease,
): Promise<OpenzroRelease | undefined> => {
  const runFetch =
    release === undefined ||
  ...
  if (runFetch) {
    const data = (await fetch(
      `${GITHUB_API_ENDPOINT}/repos/openzro/openzro/releases/latest`,
 ...
 * Routing peers are supported on Windows, macOS, iOS & Android starting from Openzro v0.36.6+.
 * @param version
 * @param os
 */
