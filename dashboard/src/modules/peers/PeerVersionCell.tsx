...
export default function PeerVersionCell({ version, os }: Props) {
  ...
  const updateIcon = useMemo(() => {
    return <ArrowUpCircleIcon size={15} className={"text-openzro"} />;
  }, []);
  ...
  return updateAvailable ? (
    <TooltipProvider>
      <Tooltip delayDuration={10}>
        <TooltipTrigger>
          <div className="flex gap-2 dark:text-neutral-300 text-neutral-500 hover:text-neutral-900 dark:hover:text-neutral-100 transition-all hover:bg-neutral-100 dark:hover:bg-nb-gray-800/60 py-2 px-3 rounded-md items-center">
            <MemoizedOpenzroIcon />
            {version == "development" ? "dev" : version}
            <div className={"relative"}>
              <span className="animate-ping absolute left-0 inline-flex h-[15px] w-[15px] rounded-full bg-openzro opacity-20"></span>
              {updateIcon}
            </div>
          </div>
        </TooltipTrigger>
        ...
        <TooltipContent>
          ...
          >
            <MemoizedOpenzroIcon />
            <span>{version}</span>
            <ArrowRightIcon size={16} className={"text-openzro"} />
            <span className={"text-openzro"}>{latestVersion}</span>
          </div>
          ...
          >
            A new version of Openzro is available. Please update your client to
