...
import { isLocalDev, isProduction } from "@utils/openzro";
...
export default function ActivityDescription({ event }: Props) {
  ...
  if (event.activity_code == "setupkey.peer.add")
    return (
      <div className={"inline"}>
        ...
        with the Openzro IP <Value>{m.ip}</Value>
      </div>
    );
  ...
  if (event.activity_code == "user.peer.delete")
    return (
      <div className={"inline"}>
        ...
        Openzro IP <Value>{m.ip}</Value> was deleted
      </div>
    );
  ...
  if (event.activity_code == "user.peer.update")
    return (
      <div className={"inline"}>
        Peer <Value>{m.name}</Value> <PeerConnectionInfo meta={m} /> with
        Openzro IP <Value>{m.ip}</Value> was updated
      </div>
    );
  ...
  if (event.activity_code == "user.join")
    return (
      <div className={"inline"}>
        User <Value>{m.username}</Value> joined Openzro
      </div>
    );
  ...
  if (event.activity_code == "peer.group.delete")
    return (
      <div className={"inline"}>
        ...
        Openzro IP <Value>{m.peer_ip}</Value>
      </div>
    );
  ...
  if (event.activity_code == "peer.group.add")
    return (
      <div className={"inline"}>
        Group <Value>{m.group}</Value> was added to the peer with the Openzro IP{" "}
  ...
  if (event.activity_code == "peer.rename")
    return (
      <div className={"inline"}>
        Peer with the Openzro IP <Value>{m.ip}</Value> was renamed to{" "}
        <Value>{m.name}</Value>
      </div>
    );
  ...
  if (event.activity_code == "peer.approve")
    return (
      <div className={"inline"}>
        Peer with the Openzro IP <Value>{m.ip}</Value> was approved
      </div>
    );
...
function Value({
  ...
  return children ? (
    <span
      className={cn(
        "inline-flex gap-1 items-center max-h-[22px] font-medium py-[3px] text-[11px] px-[5px] rounded-[4px] border",
        "bg-white text-neutral-700 border-neutral-200",
        "dark:bg-nb-gray-900 dark:text-nb-gray-200 dark:border-nb-gray-800",
        className,
      )}
    >
      {children}
