...
A Cora operator reported a hard freeze ("trava tudo") on the openZro
...
`https://ci.cora.tools` in the browser → the dashboard
(`https://dash.cora.tools`) and every other `*.cora.tools` HTTPS
...
`ci.cora.tools` triggered the loop again.

Cora's deployment puts every infra hostname under a single GKE
nginx-ingress LoadBalancer at `35.188.178.156`:

- `mgmt.cora.tools`, `access.cora.tools`, `dash.cora.tools`,
  `signal.cora.tools`, `relay.cora.tools`, `cora.tools` (apex)
- The routed apps `ci.cora.tools`, `cd.cora.tools` (sharing the
  same LB; path-routing inside the ingress dispatches per Host
  header).

`mgmt-grpc.cora.tools` is the lone exception, on its own LB —
...
With every `*.cora.tools` host resolving to the same
`35.188.178.156`, that /32 catches the management/dashboard/signal
...
covering `cora` → `All` for TCP/80,443. Once that policy was
...
internet traffic to `35.188.178.156` (which the client has been
...
the default policy in place, `dash.cora.tools` (no app-layer
allowlist) succeeded while `ci.cora.tools` (firewalled to
...
4. **Do not change the zero-trust default**. A blanket
   `cora → All TCP/*` policy would have hidden the symptom but
...
|---|---|---|---|---|
| user group (e.g. `cora`) | routing-peer group (e.g. `cora-admin-us`) | TCP | 80, 443 | The actual app traffic |
...
- Cora reproduction artefacts: shared GKE LB IP `35.188.178.156`
  serving every `*.cora.tools` host except `mgmt-grpc.cora.tools`
