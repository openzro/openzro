...
// ResolvedAddressesForResources runs a single GROUP-BY across all
// the requested resource IDs and returns a map keyed by resource ID
// → distinct dest_ips observed since `since`. One query handles the
// whole listing path (no N+1).
//
// Index strategy: the existing idx_flow_account_received compound
// is a range scan on (account_id, received_at) — it narrows the
// candidate rows hard. The dest_resource IN (...) filter and the
// DISTINCT happen as a post-scan filter / aggregation on the
// (much smaller) candidate set. No new index needed at v1
// volumes; if the query gets slow at high tier, the right move is
// a partial index on dest_resource for the
// (account_id, received_at, dest_resource) prefix.
func (s *Store) ResolvedAddressesForResources(
	ctx context.Context, accountID string, resourceIDs []string, since time.Time,
) (map[string][]string, error) {
	if accountID == "" {
		return nil, errors.New("flow/store/sql: AccountID is required")
	}
	if len(resourceIDs) == 0 {
		return map[string][]string{}, nil
	}

	// Convert resource IDs to the byte form the column stores. The
	// agent encodes route.ResID as bytes when emitting flow events;
	// matching against bytes is the wire-level equality.
	idBytes := make([][]byte, 0, len(resourceIDs))
	...
	q := s.db.WithContext(ctx).
		Model(&row{}).
		Select("DISTINCT dest_resource, dest_ip").
		Where("account_id = ?", accountID).
		Where("dest_resource IN ?", idBytes)
	if !since.IsZero() {
		q = q.Where("received_at >= ?", since)
	}

	var pairs []pair
	if err := q.Scan(&pairs).Error; err != nil {
		return nil, fmt.Errorf("flow/store/sql: resolved-addresses query: %w", err)
	}

	out := make(map[string][]string, len(resourceIDs))
	for _, p := range pairs {
		if len(p.DestResource) == 0 || p.DestIP == "" {
			continue
		}
		key := string(p.DestResource)
		out[key] = append(out[key], p.DestIP)
	}
	return out, nil
}
